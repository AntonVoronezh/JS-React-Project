...
const FieldCreatePage = ({t, addField}) => {
	const id = -Date.now();
	...
	const fieldInputs = (
		<Formik
			...
			onSubmit={(values, {setSubmitting}) => {
				...
				toast(t('toast.text', {type: t('toast.field'), action: t('toast.create')}));
			...
			render={({setFieldValue, isSubmitting, errors, touched}) => (
				<Form className="form-container  model-inputs-fields">
					...
					/>
					<Field component={Checkbox} name="isMultiple" id="isMultiple" label={t('fields.edit.isMultiple')}/>
					<Field component={Checkbox} name="isRequired" id="isRequired" label={t('fields.edit.isRequired')}/>
					<TypeSelect onChange={setFieldValue} label={t('fields.edit.type')} error={errors.type}/>
					<button type="submit" className="btn btn-success pt-20" disabled={isSubmitting || !_size(touched)}>
