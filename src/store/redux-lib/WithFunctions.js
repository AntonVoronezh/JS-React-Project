...
export class WithFunctions extends WithActionsCreators {
	...
	_createFunctions = list => {
		...
		list.map(el => {
			...
			const actionCreator = this._createActionCreators(this.typesNames)[functionName + partsOfWords.AC];
			result = { ...result, [functionName]: model => dispatch => dispatch(actionCreator(model)) };
