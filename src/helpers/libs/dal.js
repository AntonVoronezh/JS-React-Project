...
import { getFetchInstanse } from "../services";
import { bl } from "./bl";
...
class DAL {
  get = ({ object, endpoint, namespace, part, shema, payload }) => ({
    [RSAA]: {
      ...
      types: [
        ...object.getRSAAActionsAsArrayForFetch({
          namespace,
          part,
          shema,
          payload,
  ...
  setSyncWithoutFetchErrors = ({ funcForCall }) => {
    ...
    window.timerId[random] = window.setInterval(
      () => funcForCall(),
      syncTime.timeInMiliseconds
    );
  };
  ...
  setSync = ({ error, funcForCall }) =>
    error
      ? this.setSyncWithFetchErrors({ funcForCall })
      : this.setSyncWithoutFetchErrors({ funcForCall });
