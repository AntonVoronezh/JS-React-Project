...
interface IProps {
  fieldsList: any;
  fetchFields: () => void;
  setFieldsFromLocalStorage: () => void;
  errorMsg: any;
}
...
function WithFields(WrappedComponent) {
  class Wrap extends Component<IProps> {
    componentDidMount() {
      ...
      !fieldsList &&
        ui.getData({
          item: "fields",
          setFunc: setFieldsFromLocalStorage,
          fetchFunc: fetchFields,
        });
  ...
  const mapDispatchToProps = (dispatch) =>
    bindActionCreators({ fetchFields, setFieldsFromLocalStorage }, dispatch);
